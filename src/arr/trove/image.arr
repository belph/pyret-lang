...
fun snap-row-trans(img :: List<List<Number>>) -> M.Vector:
  ...
  min-x = img.first.sort().first
  min-y = img.get(1).sort().first
  trans-x = if (min-x < 0):
    num-abs(min-x)
  else:
    -1 * min-x
...
fun line(x     :: Number,
    y     :: Number,
    color :: Color) :
  polygon([matrix(2,2): 0, x, 0, y],outline,solid,posn(x / 2, y / 2)).snap-axes()
end
# text()
# Represents the given string
#   of text, using the given
...
fun rhombus(side-length :: Number%(greater-than-zero),
  ...
  beta = (pi / 2) - (to-radians(angle) / 2)
      polygon([matrix(2,3): 0, side-length * num-cos(beta), 2 * (side-length * num-cos(beta)), side-length * num-cos(beta),
...
fun num-inexact-string(n :: Number) -> String:
  ...
  # There's no documented builtin way to force floating point representation...
  if (num-is-integer(n) or
      not(strings-equal(num-tostring(n),num-tostring(num-exact(n))))):
    num-tostring(n)
  else:
    # num-floor + 1 instead of num-ceiling because you never know
    #   when some strangely-formatted power of ten may creep into
    #   this else clause
    num-tostring(n + num-sin(0.0))
  end
end

fun point-to-string(pt :: Position):
  # Because there's no to-decimal function,
  #   I had to resort to this process in order
  #   to avoid fraction forms of numbers, which
  #   are junk to SVG.
...
fun draw-svg(i :: Image) -> XML.Element:
  ...
  b = i.svg-size()
  ht = XML.attribute("height", b.height)
  wd = XML.attribute("width", b.width)
  XML.tag("svg",[list: wd, ht], pre)
end
