...
data CornerBox:
  | cornerbox(top-left :: IM_AUX.Position, bottom-right :: IM_AUX.Position) with:
    ...
    intersect(self, other :: CornerBox) -> CornerBox:
      ...
      cond1 = self.top-left.x > other.bottom-right.x
      # 2. Right edge of self left of left edge of other
      cond2 = self.bottom-right.x < other.top-left.x
      # 3. Top of self below bottom of other (> because screen coordinates)
      cond3 = self.top-left.y > other.bottom-right.y
      # 4. Bottom of self above top of other
      cond4 = self.bottom-right.y < other.top-left.y
...
where:
  ...
  pt = param(xt,yt)
  pt.at(1) is IM_AUX.posn(2, 1)
  pt.at(3) is IM_AUX.posn(6, 9)
end
...
data QuadTuple:
  | qtup(a :: Number, b :: Number, c :: Number) with:
    ...
    solve(self) -> List<Number>:
      ...
      if disc < 0:
        [list: ]
      else if (self.a == 0) and not(self.b == 0):
...
where:
  ...
  qtup(2,4,-4).solve() is%(circa) [list: (-1 - num-sqrt(3)), (-1 + num-sqrt(3))]
  qtup(1,4,5).solve() is [list: ]
  qtup(1,2,1).solve() is [list: -1]
end
...
where:
  circa = lam(l,r): fold2(lam(a,b,c): a and (num-abs(b - c) < 0.0001) end, true, l, r) end
  tup-pair(qtup(2,4,-4),qtup(1,2,1)).solutions() is%(circa) [list: (-1 - num-sqrt(3)), (-1 + num-sqrt(3)), -1]
end
...
fun cubic-bezier-bounding-box(p0 :: IM_AUX.Position, p1 :: IM_AUX.Position, p2 :: IM_AUX.Position, p3 :: IM_AUX.Position) -> CornerBox:
  ...
  in-range = lam(t): (0 <= t) and (t <= 1) end
  lessx = lam(a,b): a.x < b.x end
  eqx = lam(a,b): a.x == b.x end
  lessy = lam(a,b): a.y < b.y end
  eqy = lam(a,b): a.y == b.y end
